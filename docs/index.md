...
A tour of [contact-red/odbc](https://github.com/contact-red/odbc), a Pony wrapper around unixODBC. By the end you'll have used every major piece of the public API: connections, cursors, prepared statements, transactions, the `SqlValue` union, zero-allocation fetch loops, the async `DbSession` actor, and cross-actor cancellation.
...
!!! warning "Alpha status"
    The library is alpha and breaking changes are expected. This tutorial targets `0.0.2`; for later releases check [odbc.contact.red](https://odbc.contact.red/).
...
- **Prepared Statements** — parameter binding, statement reuse, parameterised SQL.
